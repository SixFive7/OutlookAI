...
| Capability | Tools |
|---|---|
| **Search** all cached mailboxes (including delegate/shared mailboxes and attachment contents) in milliseconds; search words are matched in both the subject and the body, and can be narrowed to just one of them; results always include mail that arrived seconds ago (a built-in freshness sweep, cached briefly so repeated searches stay instant), and an `exhaustive` option scans folders directly when the index is stale | `search`, `thread` |
