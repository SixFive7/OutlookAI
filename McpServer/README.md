...
L1 search: `search` (fast | fresh | exhaustive), `thread`, `index_status`, `list_accounts`, `list_folders` · L2 read: `read`, `save_attachment` · L3 show-me: `open_in_outlook`, `goto_folder`, `show_search_results` · L4 drafts: `new_draft`, `reply_draft`, `replyall_draft`, `forward_draft` · L5: `send` (high-friction, two-step) · diagnostics: `health`, `echo`.

Search modes: `fast` = index only (works with Outlook closed, may lag); `fresh` (default) = index + a bounded COM sweep of mail newer than the index frontier (catches just-arrived mail); `exhaustive` = a folder/date-bounded COM scan that bypasses the index entirely (requires `store` plus `folder` and/or `after`).
