...
L1 search: `search` (always fresh; `exhaustive` flag), `thread`, `list_accounts`, `list_folders`, `list_signatures`, `manage_signature` · L2 read: `read` (body windows page via `body_offset`), `save_attachment` · L3 show-me: `open_in_outlook`, `goto_folder`, `show_search_results` · L4 drafts: `new_draft`, `reply_draft`, `replyall_draft`, `forward_draft` (each with an optional `signature` override) · L5: `send` (high-friction, two-step) · diagnostics: `outlook_health` (17 tools total).
