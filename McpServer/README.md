...
L1 search: `search` (always fresh; `exhaustive` flag), `thread`, `list_accounts`, `list_folders`, `list_signatures`, `manage_signature` · L2 read: `read` (body windows page via `body_offset`), `save_attachment` · L2.5 organize: `move_mail`, `archive_mail` (same-store, audited, reversible) · L3 show-me: `open_in_outlook`, `goto_folder`, `show_search_results` · L4 drafts: `new_draft`, `reply_draft`, `replyall_draft`, `forward_draft` (each with optional `signature`, `cc`, `bcc`, `importance`, `request_read_receipt`; the three derived ones also take a `subject` override) · L5: `send` (high-friction, two-step) · diagnostics: `outlook_health` (19 tools total).
