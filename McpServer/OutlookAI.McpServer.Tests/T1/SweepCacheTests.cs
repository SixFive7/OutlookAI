...
public sealed class SweepCacheTests
{
    ...
    {
        SweepCache cache = new();
        cache.Store(Frontier, store: null, MakeResult(1), elapsedMs: 50, Now);

        Assert.False(cache.TryGet(Frontier, store: null, Now + SweepCache.DefaultTimeToLive + TimeSpan.FromMilliseconds(1), out _));
    }
    ...
    public void FrontierAdvance_InvalidatesTheEntry()
    {
        ...
        Assert.False(cache.TryGet(Frontier.AddMinutes(2), store: null, Now.AddSeconds(1), out _));
    }
    ...
    {
        SweepCache cache = new();
        cache.Store(Frontier, store: null, MakeResult(2), elapsedMs: 80, Now);

        // All-stores sweep covers any single store (caller filters items by store).
        Assert.True(cache.TryGet(Frontier, store: "someone@example.com", Now.AddSeconds(5), out SweepCache.CachedSweep? cached));
        Assert.Null(cached!.Store);

        // A store-scoped sweep must never serve an all-stores request.
        SweepCache scoped = new();
        scoped.Store(Frontier, store: "someone@example.com", MakeResult(2), elapsedMs: 80, Now);
        Assert.False(scoped.TryGet(Frontier, store: null, Now.AddSeconds(5), out _));

        // Nor a request for a DIFFERENT store.
        Assert.False(scoped.TryGet(Frontier, store: "other@example.com", Now.AddSeconds(5), out _));

        // But the exact store matches, case-insensitively.
        Assert.True(scoped.TryGet(Frontier, store: "SOMEONE@example.com", Now.AddSeconds(5), out _));
    }
    ...
    {
        SweepCache cache = new();
        cache.Store(Frontier, store: null, MakeResult(5), elapsedMs: 200, Now);
        cache.Store(Frontier, store: "someone@example.com", MakeResult(1), elapsedMs: 30, Now.AddSeconds(2));

        Assert.True(cache.TryGet(Frontier, store: "someone@example.com", Now.AddSeconds(4), out SweepCache.CachedSweep? cached));
    ...
    {
        SweepCache cache = new();
        cache.Store(Frontier, store: null, MakeResult(1), elapsedMs: 10, Now);
        cache.Store(Frontier, store: "someone@example.com", MakeResult(1), elapsedMs: 10, Now);

        cache.Clear();

        Assert.False(cache.TryGet(Frontier, store: null, Now.AddSeconds(1), out _));
        Assert.False(cache.TryGet(Frontier, store: "someone@example.com", Now.AddSeconds(1), out _));
    }
    ...
    public void ZeroTtl_DisablesTheCache()
    {
        ...
        Assert.False(cache.TryGet(Frontier, store: null, Now, out _));
    }
    ...
    private static ComSweepResult MakeResult(int items)
    {
        ...
        return new ComSweepResult(list, foldersSwept: 2, foldersSkipped: 0);
    }
}
