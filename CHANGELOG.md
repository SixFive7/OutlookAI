...
- Add a term_scope option to mail search: keep the default and a word is found in the subject or the body, or narrow it to just the subject or just the body when a word is noisy in the other one (for example a term that appears in every quoted thread or every alert subject line). The search tool description now also states plainly what a search word is matched against - and that sender and recipient are matched by the separate from/to filters, not by the search words.
