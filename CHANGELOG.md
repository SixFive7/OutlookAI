...
- Add an OutlookAI Settings dialog: a new small button on Outlook's main Mail ribbon opens a settings window (matching your Office light or dark theme) where each tuning group can be switched on or off, the current effective values are shown, and a clear indicator tells you when an Outlook restart is still needed. Turning a group off stops managing it and leaves your Outlook settings as they are.
