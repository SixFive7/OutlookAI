...
- Fix drafts coming out noticeably worse whenever Outlook is not already open on screen - which is the normal case, since the assistant starts Outlook in the background without a window. In that state Outlook refuses to hand over the editor a draft is written in, so the assistant silently produced a lesser draft: your message lost the mail's own font and layout, a specific signature you asked for was dropped and the account default used instead, the images in a signature were never embedded, the answer came back with no recipients listed, and revising a draft failed outright. Worse, finishing such a draft made the background Outlook shut down again, which then broke whatever the assistant did next. A draft written with no Outlook window open is now identical to one written with Outlook open - right signature, working images, your text above the signature and the quoted conversation, real formatting - and nothing appears on your screen while it happens. Outlook is also kept running instead of exiting after each draft.
- Report it when a draft could not be written the proper way, instead of quietly handing back a worse one. If the editor cannot be reached the draft is still created and nothing is lost, but the answer now names what that costs - the message layout, a requested signature, embedded signature images - and how to get the full result. When nothing went wrong, nothing is reported.
- Fix a draft reporting your signature's logo as an empty, zero-byte attachment. The image itself was never wrong - the picture in a new draft is the real, full-size file and arrives that way for the recipient - but the answer the assistant got back said it was 0 bytes (and in some cases showed no attachment at all), because Outlook has not finished recording the attachment's size while the draft is still being written. An assistant reading that concludes the logo is broken and the mail unusable, which is exactly what happened. Drafts now report the same file sizes the recipient will actually receive. Verified end to end by sending a real mail out to an external address and back: the logo arrives as a valid image of exactly the right size, referenced correctly by the message, with no trace of a local file path.
- Fix the image in a signature disappearing when the assistant revises a draft, and fix the same image being unusable to whoever receives the mail. The picture was never actually put into the draft: it was left as a link to the file in your Signatures folder, so it only ever displayed on this machine, and rewriting the draft replaced it with an empty placeholder box. Signature images are now embedded in the draft itself, so they survive any number of revisions and arrive intact for the recipient. A draft made by an older version still carries the old link and cannot be repaired in place - revising such a draft now says so explicitly instead of losing the image quietly, and tells the assistant to re-apply the signature, which restores it.
- Fix a search for words inside attachments also returning ordinary mail that merely matched on subject or body. The just-arrived-mail sweep cannot look inside attachments at all, yet its results were mixed in, so an attachment-only search answered with mail that had no matching attachment. Such a search is now purely attachment matches, and the assistant is told that the freshest mail is not covered by it until that mail has been indexed.
- Add draft revising: the assistant can now change a draft it already made instead of starting a second one - rewrite the text, rename the subject, change who it goes to, switch the signature, or add a file. Your message text is replaced; the account's signature and, on a reply or forward, the quoted conversation stay exactly where they were. Changing a subject keeps the draft in its original conversation. Recipients are replaced rather than added to, so a wrong address can finally be taken off, and only the things actually passed are touched. Only unsent drafts sitting in a Drafts folder can be revised - anything else is refused with a clear reason and left alone.
- Add draft discarding, so a draft that turned out wrong does not have to be cleaned up by hand. It is deliberately narrow: the assistant can only throw away a draft it itself created in the same session, that is still unsent and still in Drafts. Mail you received, mail you wrote, anything already sent, anything outside Drafts and drafts from an earlier session are all out of reach, and it can never delete permanently or empty anything. It works exactly like pressing Delete in Outlook - the draft goes to Deleted Items and the result says how to put it back.
- Add file attachments to drafts: the assistant can attach files from anywhere on the machine to any draft it creates or revises, and remove them again by name. Every path is checked before anything is written, so if one file is missing, unreadable or a folder, nothing is attached at all and the answer names each problem file - instead of quietly sending a mail with a file missing. The result reports which files ended up on the draft, with their sizes.
- Strengthen the safety check on automatic sending: the confirmation you give now also covers the attached files and the message's formatting, not just its text and recipients. Attaching or removing a file after you approved a send, or a change that alters a link while the visible words stay the same, now cancels that approval and requires a fresh confirmation.
- Add formatted drafts: the assistant can now write a message as real HTML - headings, bold, bulleted and numbered lists, tables, links and inline styling - so a proper letter finally comes out looking like one instead of a wall of plain text. The formatting goes into your message only: the account's signature and, on a reply or forward, the quoted conversation are left exactly as Outlook made them, below your text. Plain text remains the default and behaves exactly as before. Anything the assistant sends that a mail cannot safely carry - scripts, style blocks, embedded frames, images, tracking attributes - is removed, unknown tags are unwrapped with their text kept, and half-finished markup is repaired rather than rejected, so a small mistake in the assistant's output can never mangle the message or swallow your signature. Whatever was changed is reported back to the assistant.
- Let the assistant check how a mail actually looks: reading a mail can now also return its stored HTML, which is the only way to see formatting, where the signature starts and where the quoted conversation begins - the plain-text view hides all of it. This is what lets the assistant verify a formatted draft it just wrote before showing it to you, and it works on a draft directly, without needing the search index.
- Fix new email drafts putting the signature in the wrong place: when the assistant was asked to use a specific signature, the whole message could end up inside the signature area - Outlook and the writing sidebar then treated your text as part of the signature, and it lost the message's own font and layout. Drafts are now written the way Outlook's own compose window does it: Outlook inserts the account's real signature (HTML, logos and all) and your text is placed above it, leaving the signature untouched. This applies to new mail, replies and forwards alike, and an account without a configured signature still gets a body-only draft.
- Add Cc and Bcc to every draft the assistant creates - replies and forwards included, where the addresses are ADDED to the recipients Outlook already filled in rather than replacing them (a reply-all keeps everyone). Any address Outlook cannot recognize is reported back with the draft instead of being silently dropped.
- Add a subject override for reply, reply-all and forward drafts, for when a thread needs renaming. The draft stays part of the original conversation: it still groups with the thread in Outlook and its reply chain is preserved, which Outlook does not do on its own when a subject is changed.
- Add importance (low / normal / high) and a read-receipt request to all four draft tools.
- Fix mail search never finding words that live inside an image, an embedded message or a calendar invite attached to a mail: attachment contents were only searched when the attachment was a document, so a photo, a forwarded .msg/.eml, an .ics invite, audio or video was skipped and the mail carrying it could not be found by anything written inside it. On this machine that was 1,721 attachment entries across the three accounts (images, embedded messages and invites, plus audio and video) - roughly a fifth of all attachment content, and it applies to shared mailboxes exactly as much as to your own. All attachment types are now searched, and searching got slightly faster rather than slower (measured 99 ms to 85 ms across the standard query set).
- Fix mail stored in a shared (delegate) mailbox's subfolder not opening at all: search found it, but reading it, saving its attachments, opening it in Outlook or following its thread failed every time, because the search index records those folders without the folders they sit in and Outlook was asked for the wrong location. Such mail now opens normally.
- Explain what is actually wrong when a search result cannot be opened, instead of always suggesting a retry that cannot help: the assistant is now told whether the item's folder is gone from Outlook (a stale search-index entry that will keep coming back), whether the mailbox is not open in this profile, or whether the item was moved or deleted after it was indexed - each with the step that actually resolves it.
- Fix searching inside a shared (delegate) mailbox's subfolders returning nothing at all: mail in those folders is stored by the search index under the folder's own name without the folders it sits in, so asking for a path like "Inbox/2025 invoices" pointed at a location that does not exist and came back empty - with no error, as if the folder were simply empty. On this machine that silently hid about 3,900 mails across 15 subfolders of the two shared mailboxes. Those searches now find them (verified against Outlook itself: 142 of 142 and 594 of 594 mails). Two side effects of how shared mailboxes are indexed are now reported instead of hidden: if a search cannot be narrowed to the subfolders you asked for it covers the whole shared mailbox and says so, and if that mailbox has two folders with the same name the results may include both.
- Add an include_subfolders option to mail search, on by default: a folder search covers that folder and everything under it, and switching it off searches just that one folder. It works the same way in all three search paths - the index, the freshness sweep of just-arrived mail, and the exhaustive scan through Outlook - so the exhaustive scan of a folder is no longer quietly narrower than a normal search of the same folder.
- Report every way a search result can be incomplete instead of letting it look complete: a folder whose mail Outlook would not read is now reported as failed rather than counted as checked, and the freshness sweep's per-folder limit, its folder limit, the omitted folder list, skipped folders and a requested result count that was reduced to the maximum all say so in the advice. A folder search that finds nothing now also checks whether the folder path resolved at all, so a mistyped or unrecognized folder is named as the problem instead of returning a convincing empty result.
- Fix mail search failing outright on a folder whose name contains an apostrophe (for example "O'Brien"): such folders were unsearchable with a hard error and are now searched normally.
- Fix the exhaustive folder scan running past its 2-minute limit while still reporting success: the limit was only checked while reading mail, so scanning many folders that matched nothing was unbounded. It is now checked per folder, and the result reports how far the scan got.
- Tell the assistant which searches include subfolders and which do not, so a search of a folder is never quietly narrower than it looks. The rule is spelled out for the freshness sweep of just-arrived mail (which covers the four arrival folders themselves, not their subfolders) and for showing results in Outlook, where the default shows hits from the folder itself only.
