...
- Tell the assistant which searches include subfolders and which do not, so a search of a folder is never quietly narrower than it looks: a normal search of a folder covers that folder and everything under it, while the exhaustive folder scan covers only the folder you name (the assistant is now told to walk the subfolders one by one, or to scan the whole mailbox, when completeness matters). The same rule is now spelled out for the freshness sweep of just-arrived mail and for showing results in Outlook, where the default shows hits from the folder itself only.
