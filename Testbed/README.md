...
| 9a | **Before EVERY live run: rebuild the hub population** against now | `guest/Reset-HubPopulation.ps1 -Execute`, through `guest/Register-InteractiveTask.ps1 -TimeoutSeconds 3600`, straight after a guest restart - it ends by printing this guest's `dotnet test` filter and how many minutes the frontier test has left. Decided 2026-09-24; the frontier test fails on a hub it did not rebuild (runbook §4). Needs step 9's settings, whose `hubPopulationManifestPath` it reads | guest, session 1 |
