...
| 7 | Install the mail sink, the dummy account and the identity account | Sink: `guest/Install-MailSink.ps1`, from a staged package (§6 item 11 - and read it first, §2.7 of the runbook is wrong in four places). The dummy account IS the tier profile's POP3 account, made at step 4c; its POP3 password is still **by hand, once**, because the sink refuses an empty one. The signature has a script | guest |
