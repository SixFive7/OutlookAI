...
| 7 | Install the mail sink, the dummy account and the identity account | **Sink first**, so Outlook's first send/receive finds it: `host/Get-MailSinkMedia.ps1 -Execute` stages the pinned Inbucket release on the host (`MEDIA.md`, "The mail sink"), `host/Copy-ToGuest.ps1` carries it in, then `guest/Install-MailSink.ps1 -ExpectedSha256 <hash> -Execute` registers it to start with the guest and ends by proving an SMTP-to-POP3 round trip - `SINK-READY`. Reboot and `-Verify` once more. Accounts: `guest/New-TierProfile.ps1`. **The POP3 password - MEASURED 2026-09-24 on `OAI-UNINDEXED`: the sink accepts none and any, but Outlook holding none PROMPTS and never connects**, so after the account exists, with Outlook closed: `guest/New-TierProfile.ps1 -StoreSinkPassword -Execute` - proven by the sink's debug log (`read USER tier`, `read PASS any-value`), runbook §2.7 - and again once the identity account exists (measured: its first start PROMPTS too). `SINK-READY` first time on that guest; after a graceful restart the sink started 7 s after boot. Identity account: `guest/Add-IdentityAccount.ps1`, phases in its banner - **built on both guests** (`CP-09-IDENTITY-ACCOUNT` on `OutlookAI-Indexed`, `CP-10-IDENTITY-ACCOUNT` on `OutlookAI-Unindexed`) - **but bound to the identity PST's hidden root, not an Inbox, on both** (runbook §4.1 step 6, defect 4); its `-Phase CaptureStore` now refuses that, so the identity account waits on a decision about how `identity.pst` gets a real Inbox (runbook §3b, 'The identity store has no Inbox'). The signature has a script, `guest/Set-AccountSignature.ps1` - **run on `OAI-UNINDEXED` 2026-09-24 it printed `Verified` and bound the signature to the identity PST's data-file entry instead of the account**, so that account still has no signature Outlook injects (runbook §2.8b; a product defect, not fixed here) | guest |
| 7b | **Set this guest's index state, and do it BEFORE the corpus exists** | `guest/Set-OutlookIndexingDisabled.ps1`, staged with `guest/SearchCrawlScope.cs` beside it, elevated, Outlook closed. **`OutlookAI-Indexed`: `-Enable -Execute`** - puts `mapi16://{SID}/` into the crawl scope through the Crawl Scope Manager API (the documented writer; an administrator cannot write those registry keys). **`OutlookAI-Unindexed`: `-Execute`** - a user EXCLUDE rule through the same API, then the policy value. Order matters on the unindexed guest: a NON-elevated Outlook adds itself to the index within a minute of starting (see 8c), so the exclusion must be in place before one ever runs. If Outlook rows got in first, `-Execute` waits (`-PurgeMinutes`) for the indexer to purge them itself - measured 4.2 and 4.5 minutes for 20,048 rows - before it writes the policy or restarts anything, because **a restart while that purge is pending loses it** (measured: all 20,048 rows still there after a reboot); `-Execute -RebuildCatalog` discards the catalog instead. Do not restart the guest between the rule and the end of that wait Seen on 2026-09-24: a `-Verify` whose first reading fell 74 s after boot, before the delayed-start `WSearch` was Running, said `NO-INDEXER` - each reading now re-reads the service state, but if it ever says `NO-INDEXER` within minutes of a boot, re-run it once `WSearch` is Running. | guest |
| 8 | Build the corpus | `guest/Build-Corpus.ps1` | guest, session 1 |
| 8a | **Build the fixture populations** into the hub, the bystander and - once it exists - the identity store: small, tagged, deterministic, and what the hub and index tests read | `OutlookAI.RemediationTools corpus-build --population hub\|bystander\|identity`, in the corpus profile, with every such store attached to BOTH profiles while it is still empty - `Docs/live-tier-on-the-vm.md` §3b has the order and the commands, and the hub is rebuilt before every run. **FIRST RUN, `OutlookAI-Unindexed` 2026-09-24: NOT CLEAN** (runbook §4.1, step 6; checkpoint `CP-11-POPULATIONS-BUILT-WITH-FAULTS`). The hub - the tier profile's own default store - built right: 56 items in its real folders. But every build exited 1, because no received item carries a resolved owner recipient; a PST attached with `AddStoreEx` has no Inbox or Sent Items, so the bystander's and identity store's "Inbox" items went into the PST's hidden root; and failed probe rungs left 12 items in the account-less profile's DEFAULT store (Corpus B's Drafts), which no `--allow-store` named. **Fixed host-side later the same day, generator v2 - NOT YET RUN ON A GUEST:** the owner is added as a bare address the resolver accepts, and the enrichment probe addresses its item to the owner; a folder the store lacks, or has only invisibly, gets a visible stand-in (`OutlookAI-Corpus-Folder-<id>`) and the placement probe refuses an invisible target; a store that is not the profile's default is probed only with the two rungs whose item is never unsent (`InPlaceReceived`, `PostAsNote`), every first save is checked for the store it landed in and deleted there, and every other store is swept of the corpus's probe items before and after the probes and after teardown; no lookup creates a folder. The guest is rolled back before step 6 and the populations rebuilt with the fixed tool (runbook §3b) | guest, session 1 |
...
| 8c | **Crawl the stores into the index (`OutlookAI-Indexed` only)** | Outlook must be RUNNING and **NOT ELEVATED** for the index to move: `guest/Start-OutlookUnelevated.ps1 -Profile CorpusProfile`, then `guest/Set-OutlookIndexingDisabled.ps1 -Verify -SettleMinutes 1 -WaitMinutes 30 -MinimumOutlookRows 20000` until it says `INDEXED` (it waits the crawl out and says when it finished); `host/Restart-Guest.ps1 -Execute`; the same with `-Profile OutlookAI-Tier` for the tier and identity stores; then `host/Restart-Guest.ps1 -Execute -CancelLogonPrompt`, and checkpoint (`CP-10-INDEXED` on the current guest). **Measured 2026-09-24: the 20,000-item corpus is fully crawled 7.6 to 9.6 minutes after Outlook starts (four runs); the tier profile's two small stores in under three more.** Elevated does not work, and that is the whole story of why this guest was never indexed: `guest/Register-InteractiveTask.ps1` runs at `RunLevel Highest`, and an ELEVATED Outlook never touches Windows Search (`Docs/live-tier-on-the-vm.md` section 8 item 22) | guest, session 0 driving session 1 |
...
| 9a | **Before EVERY live run: rebuild the hub population** against now | `guest/Reset-HubPopulation.ps1 -Execute`, through `guest/Register-InteractiveTask.ps1 -TimeoutSeconds 3600`, straight after a guest restart - it ends by printing this guest's opt-in value and filter for step 9b, and how many minutes the frontier test has left. Decided 2026-09-24; the frontier test fails on a hub it did not rebuild (runbook §4). Needs step 9's settings, whose `hubPopulationManifestPath` it reads | guest, session 1 |
...
**Restarting a guest, at any step: `host/Restart-Guest.ps1 -VMName <guest> -Execute`, and nothing
else.** Not `shutdown /r /t 5` - Microsoft documents that any timeout above 0 implies `/f`, so that
command force-closes Outlook with its PSTs open - as near to the `taskkill` that mailbox-safety
rule 7 forbids as makes no difference. Most guest restarts on record before 2026-09-24 were that
command or the same with a longer timeout (the earlier rounds' scratch scripts). The
script quits Outlook gracefully first (and refuses - never forces - when it cannot), restarts with
no `/f`, and returns only once the console session is Active again; with the tier profile running,
add `-CancelLogonPrompt` for its POP3 logon prompt. Hold a lease around it (section 5b).
...
| `host/Restart-Guest.ps1` | **The way to restart a guest - the only one.** The restarts on record before 2026-09-24 were mostly `shutdown /r /t 5` (or 10, 15, 20), and Microsoft documents that a timeout above 0 implies `/f`: each of those FORCE-CLOSED whatever Outlook was still running, PSTs open. This one quits Outlook first, the way mailbox-safety rule 7 says, in session 1 at Outlook's OWN integrity level (it reads the process token - an elevated caller cannot see a non-elevated Outlook in the Running Object Table, nor the reverse): attaches to the RUNNING instance - through the Running Object Table first, and after 20 s through the running instance's class object, the way the product's own server attaches, because Office joins the Running Object Table only once its window loses focus and on an unattended guest nothing takes it (KB 238610, and measured: a minute of `MK_E_UNAVAILABLE` from an Outlook nine minutes up); refuses on any visible Outlook dialog, an open Inspector, a non-empty Outbox in any store, the installer mutex or a known COM client (`OutlookAI.McpServer`, `OutlookAI.RemediationTools`, `testhost`); releases every COM reference before `Quit()`; and waits for OUTLOOK.EXE to leave, never killing it. Then `shutdown.exe /r /t 0` with no `/f`, and it waits for the heartbeat to drop and return, a LATER boot time (proof a restart happened) and the console session Active. `-CancelLogonPrompt` presses Cancel on the tier profile's POP3 "Internet Email - tier" logon prompt, which that profile raises at every start; nothing else is ever clicked, and a security prompt is always a refusal. Dry run without `-Execute`. **RUN 2026-09-24 on `OutlookAI-Indexed` 18 times - 3 with an elevated Outlook up, 15 with a non-elevated one: 16 restarts, Outlook gone by the first check after `Quit()` every time and the console Active again 19 to 47 s after the request (the slower half spent 20 s on the Running Object Table before attaching through the class object); and 2 refusals that forced nothing - one behind a second POP3 logon prompt, one when it could not attach - the two cases the multi-prompt cancel and the class-object attach were then added for.** One of the 16, before its wait loop was fixed, restarted the guest and then sat waiting for a heartbeat drop it had missed; it now watches the boot time as well. `-SelfTest` (34 checks, pure) pins the session parser, the refusal codes and - by parsing this file - that it issues no forced shutdown, `Stop-VM`, `Restart-VM` or kill. |
...
| `guest/Set-OutlookIndexingDisabled.ps1` | Sets a guest's Outlook index state in BOTH directions, then proves it by asking the SERVICE and the CATALOG - never by reading back what it wrote. `-Execute` takes Outlook OUT: a user EXCLUDE rule for `mapi16://{SID}/`, then a wait for the indexer to purge any Outlook row crawled before it (`-PurgeMinutes`), and only then the documented policy `PreventIndexingOutlook = 1` and a service restart - an order that is a measured finding, not a style (below). `-Enable -Execute` puts it IN: policy 0, plus the search root and a user INCLUDE rule. Both go through the Crawl Scope Manager API in `guest/SearchCrawlScope.cs`, **which must be staged beside it**, and both are read back with `IncludedInCrawlScopeEx` and refused if the service disagrees. It never disables the Windows Search service (a machine with no indexer is the product's "index unreachable" branch, not a store with no frontier) and never writes a crawl-scope registry key (an administrator cannot - measured on Q68, which made that layer read-only). `-Verify` reports the service's own verdict and reason for the scope, every mapi rule and root it holds, the catalog's status and items-to-index counters, and - twice, `-SettleMinutes` apart - the Outlook rows PER STORE. Five verdicts; `INDEXED` now means the crawl has FINISHED (the same count on both readings, nothing queued, at least `-MinimumOutlookRows`). **RUN 2026-09-24 on `OutlookAI-Indexed`, both directions (Q69):** from a clean checkpoint `-Enable -Execute` put the scope in (`included=True reason=USER`), and with Outlook started non-elevated `-Verify -WaitMinutes 30 -MinimumOutlookRows 20000` said `INDEXED` once the crawl had finished (7.6 to 9.6 minutes, four runs; 20,048 rows over three stores). From that indexed checkpoint `-Execute` wrote the rule, watched the indexer purge all 20,048 rows itself in 4.5 minutes, then wrote the policy and restarted the service: `UNINDEXED`, reason `USER` - and still `UNINDEXED` after a non-elevated Outlook had run 5 minutes on top of it and the guest had been restarted. The order it used before - policy, rule and restart in one go - purged nothing (all 20,048 rows still there after a reboot), and neither did the rule followed by a restart; `-RebuildCatalog` then removed them at once. The policy on its own is not a crawl-scope rule at all: the service still reports the scope IN. `-Verify` now re-reads the service with every reading, after one taken seconds after a restart judged a live catalog `NO-INDEXER` on a stale "Stopped". History: the 2026-09-16 `UNINDEXED` on `OutlookAI-Unindexed` proved only that that guest held no Outlook row - it had never had Outlook in scope either. `-SelfTest` (24 checks, pure) drives the verdict table, pins that the file writes no crawl-scope registry key, that the exclusion runs rule > purge wait > policy > restart and that every reading re-reads the scope and the service, and compiles the interop and checks every IID and vtable slot. |
| `guest/Start-OutlookUnelevated.ps1` | Starts Outlook on a named profile in the interactive session **WITHOUT elevation** - a one-shot interactive task at `RunLevel Limited`, the token a user double-clicking Outlook gets - and returns once its main window is up, having read the new OUTLOOK.EXE's token and FAILED if it came up elevated anyway. Exists because an elevated Outlook never feeds the Windows Search index (measured, `Docs/live-tier-on-the-vm.md` section 8 item 22) and `guest/Register-InteractiveTask.ps1` - the only other route into session 1 - is `RunLevel Highest`. Refuses while Outlook is already running and refuses a profile that does not exist (the profile picker would be a hang). Starts Outlook and nothing else - no COM, no mail item, never quits it; `host/Restart-Guest.ps1` does that. Section 1 step 8c. **RUN 2026-09-24 on `OutlookAI-Indexed` 9 times: the Explorer window up 3 to 7 s after the start, and the token NOT elevated every time; once it refused, correctly, because an Outlook was already running.** `-SelfTest` (10 checks, pure). |
| `guest/SearchCrawlScope.cs` | **Not a script: C# source that the guest scripts compile with `Add-Type`.** The hand-declared COM interop for the Windows Search Crawl Scope Manager - `ISearchManager`, `ISearchCatalogManager`, `ISearchCrawlScopeManager`, `ISearchRoot`, `ISearchScopeRule` and their two enumerators, every method of each in `SearchAPI.h` (Windows SDK 10.0.26100.0) vtable order, each IID and each method carrying the header line it came from - plus the few helpers the scripts call: read the rules, roots and the service's own inclusion verdict, write one user rule (and its root) with `SaveAll`, read the catalog's counters, reset the catalog. These interfaces have no type library, so nothing can late-bind to them; a wrong IID fails loudly and a wrong slot silently calls a different method, which is why **two independent pins** exist: `T1/SearchCrawlScopeInteropTests` (the file is linked into the test project; start and end slot from the CLR, methods in metadata order, IIDs, against a transcribed header table) and `guest/Set-OutlookIndexingDisabled.ps1 -SelfTest` (Windows PowerShell 5.1's compiler, `Marshal.GetComSlotForMethodInfo` per method, its own IID table). Both were mutation-tested: a swapped pair, a wrong IID and a dropped method each fail both. C# 5 only; the loader strips its `#nullable` line, which that compiler rejects. **Guest-side on purpose, not in `OutlookAI.RemediationTools`**: that console also runs against the maintainer's real profile, and a writer of the machine's search scope has no business in it; `Add-Type` needs only the .NET Framework compiler every guest already has (no SDK, no .NET 10 runtime, nothing the Dependencies rule forbids), and the file is still pinned in CI because the test project links it. **Stage it beside `Set-OutlookIndexingDisabled.ps1`.** |
| `guest/Add-IdentityAccount.ps1` | Builds the **identity account** of `Docs/live-tier-on-the-vm.md` §2.8b - a second POP3 account in the tier profile with its **own** delivery store, `identity@vm.invalid`. Four phases, because they alternate between Outlook closed and running and the script never starts or stops Outlook: `-Phase Import` (renders `identity-account.prf`, `ImportPRF`), then `guest/Add-OutlookPstStore.ps1` attaches and names `identity.pst`, `-Phase CaptureStore` (reads that store's `StoreID` and Inbox `EntryID` over COM), `-Phase Bind` (writes those bytes into the account's `Delivery Store EntryID` / `Delivery Folder EntryID` - **the one undocumented step**), `-Phase Verify` (every account's delivery store and Drafts over COM, no two sharing). **RUN 2026-09-24 ON `OutlookAI-Indexed` END TO END, AND IT WORKS** - two accounts, two stores, each on its own, surviving a restart; checkpoint `CP-09-IDENTITY-ACCOUNT`. **RUN AGAIN on `OutlookAI-Unindexed` the same day, same result** (`CP-10-IDENTITY-ACCOUNT`), adding three measured facts now in its banner: the identity account prompts for its POP3 password until `New-TierProfile.ps1 -StoreSinkPassword -Execute` runs after Bind; closing Outlook needs a graceful quit, not a guest restart; and with Q80 applied `-TrySmtpAddress` reads both addresses with no prompt. A `.prf` alone cannot do it: measured the same day, Outlook binds an appended account to the profile's default store. **Both of those runs bound the account to the PST's hidden ROOT, not an Inbox** (runbook §4.1 step 6, defect 4: `identity.pst`, attached by `AddStoreEx`, has no Inbox, and `GetDefaultFolder(6)` handed back the root). **Since later that day - host-side, `-SelfTest` only, not yet run on a guest -** `-Phase CaptureStore` finds the Inbox only through the store's `PR_VALID_FOLDER_MASK` and REFUSES a delivery folder that is the root (or Top of Outlook data file, or a search root), nameless, or outside the visible tree; `-Phase Bind` refuses the same, and refuses a capture written before the check; `-Phase Verify` reads the account's `Delivery Folder EntryID` and fails on the root; and no phase creates Drafts any more (its designation is read instead). So on both guests as they stand CaptureStore refuses: how `identity.pst` gets a real, designated Inbox is open - runbook §3b, 'The identity store has no Inbox'. `-SelfTest` (39 checks, pure, under 5.1 and 7). Needs `OutlookMapiInterop.ps1` beside it. |
| `guest/Reset-HubPopulation.ps1` | **The hub's per-run rebuild - decided 2026-09-24 (question D, option (a)): a script step every run procedure calls FIRST**, the ImportPRF pattern, not a paragraph somebody must remember. Guest only, session 1, through `guest/Register-InteractiveTask.ps1 -TimeoutSeconds 3600`. Everything it needs comes from the guest's own live-test settings: the hub (`testHubStoreDisplayName`), the manifest (`hubPopulationManifestPath`, whose file name is the population id) and whether to wait for the index (the hub in `indexedStoreDisplayNames`); the seed and the old anchor come off the manifest's header. With `-Execute`: refuses a Production profile, a running Outlook (it quits none it did not start) and a pending `ImportPRF`; makes the account-less profile the default, starts Outlook and waits 180 s, `corpus-teardown --execute` by the old anchor (two keys), moves the old manifest to `hub-history\<id>.<anchor>.jsonl`, `corpus-build --execute` against NOW and reads the new header back, quits the Outlook it started under mailbox-safety rule 7 (attached through the Running Object Table, every Outbox proven empty by its mask, no open window), switches back to the tier profile, starts Outlook NOT elevated through `guest/Start-OutlookUnelevated.ps1` (an elevated Outlook never feeds the index), and on the indexed guest runs `corpus-indexed` until the index holds the whole population. It ends by printing how long the frontier test can still catch a local-time misreading - 55 min after the newest item in a W. Europe winter - and this guest's opt-in value and filter for §4c's run. Stage it beside `guest/OutlookMapiInterop.ps1`, `guest/Set-DefaultOutlookProfile.ps1` and `guest/Start-OutlookUnelevated.ps1`. Every stop says how to resume; a quit that does not complete says restart the guest and `-SkipRebuild`. **Never run on a guest**; `-SelfTest` 78 assertions under 5.1 and 7, and the guest guard refuses it on the workstation. |
...
9. ~~**Does one Windows account's Outlook profile really stay out of the index while another's is
   ...
   one tick and cannot even display the difference; the reason given for it does not. What was
   still open was whether Outlook re-creates its rule on the next start. **ANSWERED 2026-09-24
   (Q69), on a guest that IS indexed:** Outlook writes its own rule - a root and a user INCLUDE
   rule - but only when it runs NOT elevated, which no Outlook on these guests ever had (runbook
   section 8 item 22). **And an exclusion written through the API holds against it**: a non-elevated Outlook started on top of an excluded scope left it excluded (five minutes, three runs), and the rows crawled before the exclusion were purged by the indexer itself within 4.5 minutes - provided nothing restarted the service first: a restart one second after the rule, or the old policy-rule-restart order, left all 20,048 rows in place through a reboot. The policy on its own is not a crawl-scope rule at all - the service still reports the scope in - although on a guest that was never indexed it does stop a non-elevated Outlook registering itself (six minutes, no rule, no row), so `OutlookAI-Unindexed`'s recorded state - the policy only - holds against either kind of Outlook as it stands. `guest/Set-OutlookIndexingDisabled.ps1` writes the
   rule through the Crawl Scope Manager API - the rule is what excludes - and the policy beside it as a
   second layer on Outlook's side, and its `-Verify` asks the service whether the scope is in or out, and why. The 2026-09-16 background, every claim labelled by source, is in
   `.work/unindexed-guest.md`.
