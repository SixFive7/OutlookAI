...
Say ''
Say 'STILL MISSING ON THE GUEST, and this payload deliberately does not carry it: the gitignored live-test'
Say 'settings at'
Say '  McpServer\OutlookAI.McpServer.Tests\live-fixtures\live-test-settings.json'
Say 'It names that guest''s stores, so it is per-guest. Render it here once the guest''s values are recorded'
Say 'in Testbed/testbed.json - pwsh -File Testbed/host/New-LiveTestSettings.ps1 -VMName <guest> - and copy'
Say 'it in with the line that prints. Without it the live tier has no write allowlist, which is a refusal,'
Say 'not a pass.'
Say ''
