...
Closed so far: **B1**, **C1**, **C2**, **C3**, **F1**, **F5**, **G1**, **H1**, **I1** (2026-08-18), and
**C4** downgraded from SILENT to REPORTED by C1's work. **B3** is NOT closed - the item-class asymmetry
between the tiers stands - but its measurement is no longer missing: `exhaustive.rowsDropped` minus
`exhaustive.rowsUnreadable` is the count of rows the scan drops for their class, and the index tier's
`IndexSearchResult.RowsDropped` still reaches no payload (there is no `index` block on `SearchOutcome`;
adding one was deliberately not done here, to keep the `search` description inside its 2048-unit client
budget - the new codes ride on `sweep.coverageGaps`/`advice`, which the description already names).
...
| Exhaustive rows that would not open / were not mail | — | `exhaustive.rowsUnreadable` (raises `rows_unreadable`) and `exhaustive.rowsDropped` (the total; the difference is the item-class filter, B3) |
